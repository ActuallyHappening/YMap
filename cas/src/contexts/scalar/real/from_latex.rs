...
mod ir1 {
  ...
  impl From<IR1TokenFlat> for IR1TokenExpr {
    fn from(token: IR1TokenFlat) -> Self {
      IR1TokenExpr::Expr(token)
    }
  }

  pub(super) enum IR1TokenFlat {
    Num(BigUint),
    Ident(Identifier),
    ...
    pub fn from_latex_tokens(
      ...
      for token in tokens {
        ...
        match token {
          ...
          LatexToken::Ident(ident) => current.push(IR1TokenFlat::Ident(ident).into()),
          LatexToken::Num(num) => current.push(IR1TokenFlat::Num(num).into()),
          LatexToken::Bracketed(Bracketed {
            ...
            // transform only
            current
              .push(IR1TokenFlat::Bracketed(IR1TokenExpr::from_latex_not_nested(inner)?).into());
          }
          LatexToken::Frac(Frac {
            ...
            current.push(
              IR1TokenFlat::Bracketed(IR1TokenExpr::from_latex_not_nested(numerator)?).into(),
            );
            current.push(OpKind::Div.into());
            current.push(
              IR1TokenFlat::Bracketed(IR1TokenExpr::from_latex_not_nested(denominator)?).into(),
            )
          }
        }
      }
